...
pub enum VerifyX509SignatureTrustChainError {
    ...
    #[error("Signature verification for SOD DS TBS Certificate against CSCA failed: {0}")]
...
pub enum VerifyDsSignatureError {
    ...
    UnsupportedDigestAlgorithmForDsSignature(ObjectIdentifier),
    ...
    #[error("Signature verification DS signature against signed attributes failed: {0}")]
    DsSignatureVerificationFailed(ring::error::Unspecified),
...
pub enum VerifyLdsObjectHashError {
    ...
    FailedToDecodeMessageDigestForLdsObject(rasn::error::DecodeError),
...
fn verify_x509_signature_trust_chain(
    ...
    let verification_alg: &dyn ring::signature::VerificationAlgorithm =
        ...
        if signature_algorithm_oid == OID_PKCS1_SHA1WITHRSA || signature_algorithm_oid == OID_SHA1_WITH_RSA {
        ...
        // TODO: ECDSA, ED25519
            ...
            return Err(VerifyX509SignatureTrustChainError::UnsupportedSignatureAlgorithmForX509Cert(
    ...
    let tbs_certificate_der = rasn::der::encode(&subject_certificate.tbs_certificate)
        .map_err(VerifyX509SignatureTrustChainError::FailedToEncodeTbsCertificate)?;
    let key = ring::signature::UnparsedPublicKey::new(verification_alg, authority_public_key_bytes);
    key.verify(&tbs_certificate_der, &signature_bytes)
        .map_err(VerifyX509SignatureTrustChainError::SignatureVerificationFailed)
}

fn extract_message_digest_value_from_signed_attrs(
    signed_attrs: &SetOf<Attribute>,
) -> Result<&[u8], ExtractMessageDigestError> {
    for attr in signed_attrs.to_vec() {
        if attr.r#type == Oid::ISO_MEMBER_BODY_US_RSADSI_PKCS9_MESSAGE_DIGEST {
            ...
            return Ok(attr.values.to_vec()[0].as_bytes());
        }
    }
...
pub fn verify_ds_signature(signed_data: &SignedData) -> Result<(), VerifyDsSignatureError> {
    ...
    let verification_algorithm: &dyn ring::signature::VerificationAlgorithm =
        if signer_algo_oid == Oid::ISO_MEMBER_BODY_US_RSADSI_PKCS1_RSA {
                ...
                == Oid::JOINT_ISO_ITU_T_COUNTRY_US_ORGANIZATION_GOV_CSOR_NIST_ALGORITHMS_HASH_SHA512
            {
                ...
                return Err(
                    VerifyDsSignatureError::UnsupportedDigestAlgorithmForDsSignature(
        ...
        // TODO: Support ECDSA
            ...
            return Err(VerifyDsSignatureError::UnsupportedDsSignatureAlgorithm(
    ...
    let key = ring::signature::UnparsedPublicKey::new(verification_algorithm, &public_key_bytes);
    key.verify(&signed_attrs_der, signature_bytes)
        .map_err(VerifyDsSignatureError::DsSignatureVerificationFailed)
}
...
pub fn verify_lds_object_hash(signed_data: &SignedData) -> Result<(), VerifyLdsObjectHashError> {
    ...
    let expected_message_digest = rasn::der::decode::<OctetString>(
        extract_message_digest_value_from_signed_attrs(signed_attrs)?,
    )
    .map_err(VerifyLdsObjectHashError::FailedToDecodeMessageDigestForLdsObject)?;
    ...
    let lds_obj_bytes: &[u8] = signed_data
        ...
        .as_ref();
    let actual_digest = ring::digest::digest(hash_algorithm, lds_obj_bytes);
    if expected_message_digest == actual_digest.as_ref() {
        Ok(())
    } else {
        Err(VerifyLdsObjectHashError::UnexpectedLdsObjectHash {
            expected: expected_message_digest.as_ref().to_vec(),
            actual: actual_digest.as_ref().to_vec(),
        })
    }
}
...
pub fn verify_dg_hash(
    ...
    let hash_algo = if hash_algo_oid == OID_SHA256 {
        &ring::digest::SHA256
    ...
    let calculated_hash = ring::digest::digest(hash_algo, dg_data);

    if calculated_hash.as_ref() != dg_hash.data_group_hash_value {
        return Err(VerifyDgHashError::DGHashMismatch {
            data_group_number: dg_num,
            expected: calculated_hash.as_ref().to_vec(),
            actual: dg_hash.data_group_hash_value.to_vec(),
        });
    }
...
pub fn verify_sod_bytes(
    ...
    let csca_cert: X509Certificate = rasn::der::decode(csca_cert_bytes)
        .map_err(|e| PassportVerificationError::FailedToDecodeCscaCertificate(e))?;
